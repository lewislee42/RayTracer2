...
thread float Loki::rand() {
	...
	// added fix from loki github issue: https://github.com/YVin3D/Loki/issues/3
	old_seed = fract(sin(dot(float2(tid.x, tid.y), float2(12.9898,78.233))) * ((tid.x + tid.y) * old_seed));
    return old_seed;
}
